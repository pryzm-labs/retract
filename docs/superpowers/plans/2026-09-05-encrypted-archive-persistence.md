...
All seven tasks have committed implementations and independent task reviews, including the app-wide credential lease, lazy worker, terminal shutdown, indexed queries and source removal. The reviewed fix at `08a67cc` passes 313 application, 36 domain, 158 frontend and 7 release tests on both Linux Docker architectures, plus formatting, strict Clippy and production/public bundle checks. The real-worker 100,000-item corpus and its limits are recorded in [archive storage](../../ARCHIVE_STORAGE.md).

The whole-branch review found two acceptance gaps: reverse-reference checks must cover conversation parents and resource kinds, and all per-item limits must run before worker queue admission. Their combined fix at `08a67cc` has a clean independent scoped re-review, with genuine failing-then-passing tests for both findings. The original task checklists below remain the implementation requirements; this checkpoint records actual progress.

The stage is **not complete at this checkpoint**. Feature-branch push and Secure build dispatch are authorized; no main merge, PR, tag or release is implied. The initial dependency gate [Secure build run 33979891484](https://github.com/pryzm-labs/retract/actions/runs/33979891484) passed at `cdcc083e8c604fba58b13ecb85e883276e3ec40a`, including both Linux architectures, 12 native macOS codec tests and unsigned Apple-silicon package/link validation. That earlier success does not replace the final reviewed commit's complete synthetic archive/vault and unsigned-package gate. Actual Keychain prompts, ACLs and denial behavior remain separately authorized manual checks before enabling an importer. No real credentials or Telegram data were used, and no archive importer or frontend command is enabled.

### Separate follow-up

The final review classified FoundationStore's existing bare-File lock lifetime as a non-blocking P3: a briefly inherited/duplicated file description can retain a conservative `ProfileInUse` result after store closure. This does not allow concurrent writers; retry after the other description closes recovers. A separate foundation change should add explicit unlock ownership with a deterministic retained-description test. The archive store already has that pattern and regression; this stage does not alter FoundationStore.

The first post-fix amd64 full run hit `ProfileInUse` in the unchanged provider immediate-reopen test, consistent with this observation, although the specific inherited descriptor was not instrumented. The unchanged isolated test and one complete rerun passed; the issue is not claimed fixed. The arm64 gate also had one existing frontend background-refresh timing failure before passing isolated and in the complete rerun. Both transient failures remain disclosed; no tests were skipped or serialized to make the gate pass.
...
- Review-fix scope: narrow codec/internal-open-policy changes and focused `preflight.rs`/`preflight_tests.rs` are permitted to validate existing stores without mutating unsupported originals. Clean stores require side-effect-free keyed validation; recovery-sidecar cases use owner-only encrypted staging before originals are opened read-write. Supported crash recovery must still work. Bound copies, reject unsafe or observably changing snapshots, and never replace originals from staging or resurrect interrupted leftovers.
...
- [ ] Hold the process lock for the connection lifetime; serialize writes and maintenance. Forbid symlink paths and insecure file permissions. New schema creation is transactional; unsupported/corrupt existing files remain unchanged. Store references without requiring out-of-order imported targets to exist yet, but never permit a proven foreign provider/account or wrong-kind target. Check both arrival directions for content conversation/author/reply/thread relationships and conversation parents, including targets arriving after the referring source is Ready. Preserve genuinely unresolved optional IDs and valid shared identities across sources in one account. Any minimal reverse-reference index requires query-plan evidence and a deliberate schema-fingerprint update.
...
**Files:**
- Create: `src-tauri/src/persistence/archive/{ingest,ingest_tests,ingest_state}.rs`, with focused `ingest_state_tests.rs` if needed. Keep persisted run/provenance/checkpoint/restart lifecycle helpers separate from batch normalization and observation writes; helpers share the caller's transaction, never independent append commits or nested store locks.
- Modify: `src-tauri/src/persistence/archive/{model,mod,store}.rs` and test support.
- Modify narrowly: `src-tauri/src/persistence/archive/{schema,store_tests}.rs` for persisted import attempt/provenance/checkpoint fields, batch receipts and exact timestamp ordering columns/index. This remains an unreleased initial schema; older development fingerprints must be rejected unchanged, not silently accepted or reset. Validate against this binary's expected schema definition as well as checking the actual stored schema; a database matching only its own historical fingerprint is not sufficient.
...
- `ImportProgress { phase: ImportPhase, committed_items: u64, committed_bytes: u64, next_batch: u64 }`; phases `Importing`, `Ready`, `Interrupted`, `Cancelled`, `Failed`.
- `begin_import(&mut self, scope: &Scope) -> Result<ImportSession, ArchiveError>`; `append_batch(&mut self, session: &ImportSession, sequence: u64, batch: ImportBatch) -> Result<ImportProgress, ArchiveError>`; `finish_import`, `cancel_import`, and `retry_import` all require matching provenance and return durable progress/session state. Sequence starts at zero and is explicit; payload equality alone cannot distinguish retry from an intentional later update.
- Keep a stable persisted run identity and rotate the private mutation-session ID on explicit retry. A bounded backend-only status/checkpoint read must make interrupted progress discoverable after reopening without manufacturing a mutation grant. Retry accepts the expected scope, fingerprint, schema profile and run/checkpoint from that status, validates them against durable state, and returns a new session; stale checkpoints and old mutation-session IDs fail closed. No frontend-deserializable grant or new native command.
- Provide a narrow backend-only per-attempt atomic cancellation signal shared with the private session. Setting it performs no database work and must not wait for a worker queue. Append checks it before work and immediately before commit; explicit retry creates fresh attempt authority and a fresh signal. Durable cancel transitions remain repository-owned. Cover deterministic cancellation before commit with complete rollback; Task 7 supplies the queue/full-queue integration tests.
- Persist scoped `(run, sequence, digest, original committed checkpoint)` receipts in an indexed table in the same transaction as records/counters. The next sequence accepts new input; an older sequence with the exact bounded encoded batch digest returns its original committed checkpoint without mutation or quota charge. Changed payload at an old sequence, gaps and empty batches are rejected. Retain receipts across explicit retry of the same run and delete them with that source. Nonempty batches remain charged to the cumulative encoded-input ceiling; do not retain receipt arrays in memory or unbounded JSON. A source containing no records can begin and finish without appending an empty batch.
...
- [ ] Include embedded conversation participants in record-count and adapter-validation bounds; their actor observations must agree with explicit actors, not bypass duplicate-conflict checks. Preserve normalized evidence as archive-only, including nested actors, and never accept caller-supplied live evidence or findings as authority.
...
- [ ] Store timestamp ordering as signed Unix seconds plus nanosecond subsecond integers, preserving the complete normalized timestamp in record JSON. Do not use variable-precision RFC3339 string ordering or silently truncate precision. Test whole/fractional-second and pre-epoch ordering with independently listed expectations; Task 5 consumes these exact ordering columns for ranges/cursors.
...
- [ ] Test exact old-sequence replay after a later batch and after reopening/explicit retry, changed-data replay rejection, skipped sequence rejection, intentional equal data at a new sequence being charged normally, and retry token rotation. Assert receipts/counters/observations/FTS roll back together on failure and that unsupported older development schema fingerprints remain untouched.
...
- Modify narrowly if query planning requires it: `src-tauri/src/persistence/archive/{schema,store_tests}.rs` for minimal source-scoped filter/order indexes and the deliberately updated expected schema fingerprint. Justify indexes with focused query-plan/behavior checks, not speculative combinations. Reuse normalized JSON expressions where suitable rather than changing ingestion records; ask the controller before introducing a new projection/write contract.
...
- Bind cursor generation to the published import run UUID plus checkpoint revision, read in the same transaction as readiness and results. Task 4 permits mutations only while Importing and creates a new run for a newly registered source; Ready is terminal. Reuse that durable generation instead of adding redundant source-generation state. A removed source or a reused source UUID with a new run must reject the old cursor.
...
- [ ] Bound archive `resolve` to the same 200-item ceiling as query pages; reject larger reference lists before copying or looking up records. Apply the existing 64 KiB provider-envelope ceiling to locator payloads before adapter interpretation. Cover 200/201 references and oversized locators without changing Telegram's query-port contracts.
...
- [ ] Remove the affected archive account's private metadata only when no surviving source or catalog identity references it; retain other sources' account data. The intentionally content-free removal receipt survives without an account foreign key. Do not alter FoundationStore or live account state.
...
- [ ] The initial guarded migration helper accepts only clean old files; recovery sidecars reject unchanged. Keep supported normal-open recovery separate. Validate the candidate under its key and intended schema/data before publication while holding the same store process lock. A failure after rename (such as directory-sync uncertainty) must not restore older content; an authenticated valid active file remains authoritative over an obsolete candidate.
...
**Files:**
- Create: `src-tauri/src/persistence/archive/{worker,worker_tests,lifecycle,lifecycle_tests}.rs`, `src-tauri/src/secure_store/{vault_lock,vault_lock_tests}.rs`, and `src-tauri/examples/archive_storage_bench.rs`. Keep lazy app ownership/open/shutdown in lifecycle.rs and command execution in worker.rs.
...
- Modify narrowly if needed: `src-tauri/src/persistence/archive/{model,query}.rs` to share existing pure request/envelope bounds between repository and worker entry points. Reuse the existing batch bounded-size helper; do not duplicate limit constants or loosen repository validation. The shared batch helper must cover all per-item text/display-name, attachment-count and envelope/locator limits, including nested participant actors, not just aggregate batch size. Oversized batch items, query, resolve, registration and checkpoint payloads must reject before queueing or copying, as well as at their existing repository boundary. With either free or saturated worker capacity, invalid oversized items return `LimitExceeded` synchronously without accepting a command.
- Modify narrowly: `src-tauri/src/persistence/archive/{store,ingest_state}.rs` to call the shared registration/checkpoint/provenance bounds at their existing repository entry points, and `error.rs` for a payload-free internal Busy error. Queue saturation maps to existing provider Transient; actual input/source limits stay LimitExceeded and stopped services return Cancelled. No new neutral/IPC error contract.
- Modify narrowly if required by terminal shutdown integration: `src-tauri/src/provider_service.rs` and existing compatibility settings/lifecycle test seams. Preserve legitimate setup/settings retry and IPC contracts; stopped app services cannot authorize recreation, and exclusive service coordination spans archive drain and terminal cache closure.
...
- `ArchiveOwner` is app-owned and captures immutable root/opener/validator configuration once. Its Unopened/Open/Closed state tracks the sole service before returning it; open accepts no new path/key/validator configuration. A coordination-only async mutex may span awaiting off-thread open/join, but no blocking work runs on the async/UI thread and ordinary queries do not acquire that lifecycle mutex. Shutdown permanently closes the owner and drains its worker before cached vault secrets/lease are cleared; retained service handles cannot reopen it.
- A failed first open remains cached for that owner lifetime; repeated open calls do not repeat credential/filesystem I/O. Correcting a first-open failure requires restarting the app in this backend-only stage. Document this separately from explicit interrupted-import retry and transient queue backpressure; test repeated failure, shutdown drain and terminal close with injected I/O.
...
- Bind the credential lease to the same explicit application-owned root during backend setup; do not guess the root by stripping profile names. Binding alone creates no files and reads no credentials. The existing synchronized vault acquires an exclusive, fail-fast process lease before its first I/O and holds it until cached secrets are cleared. Every Telegram and archive vault consumer uses it. Store-opening key loaders acquire their profile/archive lock before entering the vault cache; never acquire a store lock or call back into a store while holding the vault-cache mutex. A cache-lifetime lease may already be held from an earlier settings or Telegram operation when a new archive store opens; reusing it is expected, not a second acquisition. All process file-lock acquisitions remain fail-fast, including contention in this ordering. Keep the non-macOS production credential behavior unchanged.
...
- [ ] Cover settings-first vault access followed by Telegram/profile or archive opening, archive-first access followed by Telegram settings, and a contending second process. Preserve the existing settings/setup behavior and prove contention returns without vault I/O or a blocking lock cycle; do not release the cached-vault lease between operations just to impose a global chronological lock order.
...
- [ ] Shutdown wakeup must remain reliable when both channel slots are reserved but no commands are sent; cancelled waiters cannot lose completion ownership. Test held reservations through shutdown and lock release, plus a settings request queued during blocked archive drain that must neither save/create a replacement service nor reopen after terminal cache closure.
...
- [ ] Keep the benchmark entry point narrow: it may accept the example's exclusively owned disposable output directory and construct synthetic adapters/keys internally under the opt-in feature. It must use the actual repository/worker implementation, never production credentials, Telegram files, or a parallel benchmark-only storage implementation. Run locally only inside Docker and verify normal packaging does not enable the feature.
...
- [ ] Run all automated gates on Linux arm64/amd64; rerun required macOS archive/vault-test/package CI on final committed code if authority permits. Native archive tests include the original codec tests and the new store/import/query/removal/worker lifecycle tests with synthetic files and disposable injected keys; vault/lease tests use injected I/O only. Keep the large benchmark opt-in rather than repeating it in every gate. Complete task review and whole-branch review; record remaining native/manual gates explicitly. Commit and offer normal branch integration, not an automatic merge/release.
