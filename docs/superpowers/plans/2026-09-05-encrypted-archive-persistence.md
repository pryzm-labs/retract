...
- Modify narrowly: `src-tauri/src/persistence/archive/{schema,store_tests}.rs` for persisted import attempt/provenance/checkpoint fields and batch receipts. This remains an unreleased initial schema; older development fingerprints must be rejected unchanged, not silently accepted or reset.
