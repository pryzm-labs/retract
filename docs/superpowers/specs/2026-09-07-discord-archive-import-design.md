...
Transcripts are arrays (including empty arrays) of objects with required `ID` (original canonical positive `u64` JSON integer token), `Timestamp`, `Contents`, and `Attachments` strings. Attachments remain one opaque string; Task 2 neither splits URLs nor decodes messages. The exact timestamp grammar is calendar-valid `YYYY-MM-DD HH:MM:SS`, seconds precision, **unzoned**. Detection performs no conversion. Synthetic fixtures use fixed unzoned calendar components, superseding the earlier proposed fixed-UTC wording; expected records contain lexemes/components, not UTC instants. Task 3 is gated: obtain authoritative UTC interpretation evidence, potentially validated against Discord's documented Snowflake UTC millisecond timestamp, or reject the unzoned profile. It must not silently assume UTC or use host-local timezone. Detection currently caps all selected evidence entries with the existing selection ceiling, conservatively limiting the number of paired contexts below that ceiling.
...
Attribute sent records to the export account; if the profile explicitly supplies a contradictory author, fail. Preserve Unicode, multiline text and attachment-only messages. Accept only the profile's specified timestamp forms, normalize explicit time zones to UTC, and never use the host's local timezone or current time as a substitute. The frozen unzoned profile is blocked from message normalization until Task 3 establishes an authoritative UTC interpretation; otherwise reject it. Missing/invalid required timestamps or IDs fail the source.
