...
Task 2 freezes `discord.data_package.messages_json` version `1`, policy `discord.import_policy.v1`; the exact synthetic contract is in `src-tauri/test-fixtures/discord-import/manifest.json`. Required entries are `Account/user.json` (object with string `id` and `username`), `Messages/index.json` (bounded ignored object), and paired `Messages/c<canonical-positive-u64>/channel.json` and `messages.json` entries. Channel objects require decimal-string `id` matching the folder and opaque string `type`; `name` may be absent/string/null, `recipients` absent/array of opaque strings, and `guild` absent/object with decimal-string `id` and string `name`. Guild-shaped headers require a string channel name and exclude recipients. No private discriminator value is interpreted, and no friendly semantic type is inferred. The account entry alone is the identity authority; index/recipient metadata cannot override it. Reserved identity/transcript filename lookalikes or wrapped roots reject; unrelated entries and bounded unknown JSON fields are ignored.

Transcripts are arrays (including empty arrays) of objects with required `ID` (original canonical positive `u64` JSON integer token), `Timestamp`, `Contents`, and `Attachments` strings. Attachments remain one opaque string; Task 2 neither splits URLs nor decodes messages. The exact timestamp grammar is calendar-valid `YYYY-MM-DD HH:MM:SS`, seconds precision, **unzoned**. Detection performs no conversion. Detection currently caps all selected evidence entries with the existing selection ceiling, conservatively limiting the number of paired contexts below that ceiling.

Task 3's authoritative interpretation ruling uses [Discord's official Snowflake reference](https://docs.discord.com/developers/reference#snowflakes), which defines message IDs as Snowflakes and their UTC milliseconds as `(snowflake >> 22) + 1420070400000`. Accept the unzoned timestamp only when its validated calendar components equal that Snowflake UTC date and time through seconds precision. On a match, normalize to the full Snowflake millisecond instant; reject mismatches, invalid calendars, overflow and out-of-range IDs. Never silently assume UTC or consult host-local time. Synthetic message Snowflakes have been regenerated for the fixed `2030-01-02 03:04:05` calendar and `.123` millisecond instant; expected records preserve both the lexeme/components and the validated UTC milliseconds. The supported profile has no author field: explicit author metadata is rejected pending separate format evidence, so it can never override the export account.
...
The typed reader inherits the detector's cumulative retained/header allocation charge through a private inspection field; rereading headers and preparing selections cannot create another retained-memory allowance while inspected headers coexist. JSON token work begins a new allowance at the detection-to-typed-read phase boundary, then stays cumulative across all typed-pass entries, headers and messages. Repeated reads retain the inventory's existing observed-byte accounting, conservatively including detection and typed reads on the same inventory. All ceilings remain lowerable and unchanged.
...
Task 4 ruling: the adapter exposes a backend-only `VerifiedConversationKind` seam for explicitly verified Direct, GroupDirect, GuildChannel and Other context. The frozen reader's opaque `source_type`, names, recipient counts and optional guild object establish no classifier; parser-originated contexts therefore remain `Other` with a fixed warning. A later evidence-backed classifier may supply the seam. Cost: current imports cannot advertise more specific conversation semantics until that evidence exists.

Attribute sent records to the export account; if the profile explicitly supplies a contradictory author, fail. Preserve Unicode, multiline text and attachment-only messages. Accept only the profile's specified timestamp forms, normalize explicit time zones to UTC, and never use the host's local timezone or current time as a substitute. The frozen unzoned profile uses the Snowflake consistency rule in section 2, retaining full UTC millisecond precision only after its exported calendar components match through seconds. Missing/invalid required timestamps or IDs fail the source.
...
Task 4 ruling: the current opaque attachment string accepts only empty or exactly one syntactically valid HTTPS URL. Preserve that URL exactly in source observation metadata; emit one attachment with unknown media and a stable message-plus-zero-based-ordinal locator. Reject ambiguous/multiple URLs, non-HTTPS and active/local scheme strings without fetching or opening anything. Cost: packages with another attachment encoding fail closed pending evidence and an explicit profile revision.

Task 4 validation reuses the archive's existing bounded encoded-size counter and named storage ceilings through crate-private visibility. Application archive construction registers the Discord payload validator lazily; it opens neither a database nor Keychain and does not register usable Discord provider capabilities.
