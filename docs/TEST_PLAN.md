...
Keep Discord unavailable to users until the source-selection and remediation UI stage. The backend automated gate includes the frozen profile/privacy audit, malformed ZIP/JSON and injected-ceiling suites, typed normalization, v1-to-v2 encrypted migration, exact replay/duplicates, hidden failures, full-queue cancellation, retained-file changes and shutdown ownership. Success/failure diagnostics are tested with synthetic content/path sentinels. Architecture controlled violations cover parser dependencies/codecs, Discord network/browser/credential coupling and command/registry exclusion; production bundles reject Discord fixture/benchmark markers.
